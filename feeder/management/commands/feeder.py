...
class Command(BaseCommand):
    ...
    def feed_report(self, report_data):
        ...
        researchtypes = ResearchTypes.objects.filter(regions=region_obj)
        researchtype_obj = random.choices(researchtypes)[0]
        products = Product.objects.filter(regions=region_obj)
        ...
        while start_date <= end_date:
            ...
            sector_obj = Sector.objects.get(id=start_date.weekday() + 10, regions=region_obj)
            companies = Company.objects.filter(sector=sector_obj, regions=region_obj)
